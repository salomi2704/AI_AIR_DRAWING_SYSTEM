"""Unit tests for the interaction-mode resolver in core.modes."""
...
class ResolveModeCoreTest(unittest.TestCase):
    ...
    def test_erase_uses_configured_hand_index(self) -> None:
        ...
        self.assertEqual(eraser.cursor, (config.ERASE_HAND_INDEX, 0.0))

    def test_erase_honours_explicit_hand_index(self) -> None:
        states = [state(Gesture.FIST, cursor=(i, 0.0)) for i in range(3)]
        _, _, eraser = resolve_mode(states, hand_index=0)
        self.assertEqual(eraser.cursor, (0.0, 0.0))
